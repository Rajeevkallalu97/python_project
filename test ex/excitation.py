...
harmonic = int(fields[0])
amplitude = float(fields[1]) # This is as a fraction of the maximum amplitude 1 = 2.96 V 
stable = float(fields[2]) #stable duration in seconds
sample_rate = int(fields[3]) #Doesn't necessarily work for other sample rates
duration = float(fields[4]) # recording duration in seconds
frequency = float(fields[5]) # Frequency
v1 = float(fields[6]) #Stable "Voltage" actually a fraction of max output positive values only 
v2 = float(fields[7]) #Recording Start "Voltage" actually a fraction of max output 0.1 = ~0.045V
v3 = float(fields[8]) #Recording stop "Voltage" actually a fraction of max output 1.0 = ~1.265
